...
## 3년동안 생각한거 헛소리 모음(진짜 헛소리가 99.9%라서 이제부터 거르는 중)
1. 기본 정보를 벡터(점)가 아닌 도형으로 생각하려는 시도가 있었다
    - 뉴럴네트워크의 흐름이 벡터 > 벡터임에 착안해서 도형 > 도형의 방법으로 네트워크를 짜려는 시도가 있었음
    - 도형을 만드는 과정을 원자와 단백질의 원리에 착안해서 만드려는 시도가 있었음
    - 위의 방법을 쓰기 위해 공간을 스프링과 노드로 구현해서 중력과 전자기력을 구현하고자 했음
    - 노드 : 좌표 에지 : 스프링
    ...
    - 각 노드가 서로 당기는 힘이 있다고 가정
    - 그 힘이 매우 강력한 노드끼리 모이면 그 노드들의 집합면이 구성됨
    - 그러한 구조체는, 매우 좁은 공간에 많은 노드가 모여 있으므로, 주변 노드를 당기려는 힘이 매우 강할수밖에 없다 = 강력한 중력
    - 좁은 공간에 많은 노드가 모여 있는 상태는 큰 질량을 가진 것으로 이해할 수 있음
    - 시작에서 발생한, 서로 당기는 힘은 각 노드가 자신끼리 당기는 새로운 힘이 있는 것으로 이해할 수 있음?
    - 중력이 약한 이유는, 만약 한 공간 안으로 스프링이 모이게 하려면 강한 힘이 필요하지만
    - 그렇게 당겨버리고 나면 외부의 모든 스프링들은 적당히 그 힘이 분산되어 서로 늘어나니까 외부에서 느끼는 중력은 약해진다
    - 스프링은 계속해서 자신의 연결을 끊고, 다시 재연결할 수 있다. 즉, 노드는 자신의 스프링 연결 상대방을 계속해서 교환할 수 있다!
    - 집합체가 되면, 그 표면 외부의 모든 노드의 스프링이 미치는 범위를 그 구조체의 특성으로 할 수 있다
    - 공간이 레이어처럼 되어있다고 생각하고
    - 각 레이어마다 스프링의 종류와 길이가 다르고
    - 하위 레이어의 결과를 상위 레이어의 노드가 가져가는 식으로
    - 스프링이 그떄그떄 연결되는게 아니라
    - 노드의 위치가 먼저 결정이 되고
    ...
    - 상위 레이어로 표면노드의 각 노드가 가지는 합력상태, 표면 형태?를 보내면 될 거
    - 대신 각 집합체들은 이동이 가능해서, 서로 당기는 것ㄱ끼리는 공간상에 가까운 위치에 있을 수 있지 않을까
    - 공간상 가까운 곳에 존재하면, 모든 공간을 탐색하지 ㅏㅇㄶ고 자신의 근처만 탐색하는 방식으로 정보를 연결할 수 있다 
    - 마지막 레이어는 최종 노드들끼리 관계성을 바탕으로 저장되어 있는, 유동성이 존재하지만 적은, 그러한 공간이다
    - (워드벡터의 아이디어를 쓰면 공간의 초기 상태를 구현하기 쉽다)
    - 새로운 정보가 들어오면 이 레이어에 정보를 쓴다
    - 기존의 워드벡터는 각 노드끼리의 관계성을 흐름으로 나타내는 네트워크 하나가 있었다
    - 이걸 바꾼다
    - 각 노드 자체가 외부로 노출하고 있는 정보들이 존재하며, 그 정보의 존재에 의해 노드는 유동적으로 움직인다
    - 이 정보를 보고 노드의 위치를 조금씩 움직여주는 네트워크가 존재할 것인가 vs 이 노드의 위치를 움직여주는 공간이 존재할 것인가 vs 둘 다?!
...
9. rnn : 순서대로 작동하는 일련의 과정이 데이터로 존재하면(지도)
...
10. reinforcement learning : 순서대로 작동하는 일련의 과정이 데이터로 존재하지 않으면(비지도)
